...
## Overview
Build a real-time car plate scanning system using Python FastAPI with EasyOCR, continuous webcam scanning, and smart deduplication to prevent duplicate API calls for recently scanned plates.
...
- [x] Set up FastAPI project with requirements.txt and Pydantic config
- [x] Implement PlateCache service with TTL-based deduplication
- [x] Implement async OCR service using EasyOCR
- [x] Build web interface with continuous webcam frame streaming
- [x] Create async httpx client for external validation API
- [x] Implement fine generation and database storage
- [x] Create POST /api/scan-frame endpoint with cache-first logic
- [x] Build live dashboard showing scanned plates and status
